...
function buildDrawerEntries(items) {
  refs.drawerContent.innerHTML = "";
  if (!items.length) {
    refs.drawerContent.innerHTML = `<div class="empty-state">Sem itens configurados.</div>`;
    return;
  }
  ...
  items.slice(0, 6).forEach((item) => {
    ...
    row.innerHTML = `
      ...
      <div class="actions">
        <a href="${item.url}" target="_blank" rel="noopener noreferrer"><button type="button">Abrir</button></a>
        <button type="button" class="copy">Copiar</button>
      </div>
    `;
    row.querySelector(".copy").addEventListener("click", () => copyText(item.url || ""));
    refs.drawerContent.appendChild(row);
  });
}

function openDrawer(type) {
  if (type === "planilhas") {
    refs.drawerTitle.textContent = "Planilhas";
    buildDrawerEntries(db.planilhas || []);
  } else {
    refs.drawerTitle.textContent = "Links Rápidos";
    buildDrawerEntries(db.links_rapidos || []);
  }
...
function exportData() {
  ...
  a.href = url;
  a.download = `dados_export_${stamp}.json`;
  document.body.appendChild(a);
  a.click();
  a.remove();
  URL.revokeObjectURL(url);
  showToast("Base exportada.");
}
...
function importDataFromFile(file) {
  ...
  reader.onload = () => {
    try {
      ...
      showToast("JSON inválido para importação.");
    }
  };
...
function bindEvents() {
  ...
  refs.menuButton.addEventListener("click", () => refs.quickMenu.classList.toggle("hidden"));

  refs.quickMenu.querySelectorAll("button[data-panel]").forEach((b) => {
    b.addEventListener("click", () => {
      openDrawer(b.dataset.panel);
      refs.quickMenu.classList.add("hidden");
    });
  });
  ...
  refs.importDataBtn.addEventListener("click", () => {
    refs.jsonImport.click();
    refs.quickMenu.classList.add("hidden");
  });

  refs.jsonImport.addEventListener("change", (e) => {
  ...
  refs.drawer.addEventListener("click", (e) => {
    if (e.target === refs.drawer) closeDrawer();
  });
  ...
  document.addEventListener("click", (e) => {
    if (!refs.menuButton.contains(e.target) && !refs.quickMenu.contains(e.target)) {
      refs.quickMenu.classList.add("hidden");
    }
  });
}
